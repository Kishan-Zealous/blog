<nav class="md:flex md:justify-between md:items-center">
    ...
    <div class="mt-8 md:mt-0 flex gap-8 items-center">
        ...
        @auth
            ...
            <div class="relative text-xs font-bold uppercase " onclick="dropdown()">
                <p>
                    {{ auth()->user()->name }}
                </p>
                <div id='userOptions' class="hidden absolute w-full bg-gray-300 rounded-md p-2">
...
<header class="max-w-xl mx-auto mt-20 text-center">
    ...
    <div class="space-y-2 lg:space-y-0 lg:space-x-4 mt-8 flex">
        ...
        <x-dropdown :options="$categories" :selectedOption="$currentCategory ?? null" path="category" optionName="Category" />

        <x-dropdown :options="$authors" :selectedOption="$currentAuthor ?? null" path="author" optionName="Author" />

        <!-- Search -->
        <div class="relative flex lg:inline-flex items-center bg-gray-100 rounded-xl px-3 py-2">
            <form method="GET"
                action="/?search={{ request('search') }}&{{ http_build_query(request()->except('search', 'page')) }}">
